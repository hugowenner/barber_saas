...
export function AppointmentsClient({ initialAppointments, barbers }: Props) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Agendamentos" description="Gerencie e acompanhe todos os agendamentos." />
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4 sm:flex-row sm:items-end">
        ...
        <div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="space-y-1.5">
            <label htmlFor="filter-date" className="text-xs uppercase tracking-wider text-muted-foreground">Data</label>
            <Input id="filter-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} className="h-10 bg-background" />
          </div>

          <div className="space-y-1.5">
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Barbeiro</label>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          ...
          {date && <> em <span className="text-foreground">{formatDateLabel(date)}</span></>}
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description={hasFilters ? "Tente ajustar os filtros para ver mais resultados." : "Não há agendamentos para esta data."}
        />
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-secondary/40">
                <tr className="text-left">
                  <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">Horário</th>
                  <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">Cliente</th>
                  ...
                  <tr key={a.id} className="transition-colors hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3">
                      <Select
                        ...
                        disabled={isPending}
                      >
                        <SelectTrigger className="h-8 w-36 border-0 bg-transparent p-0 text-xs">
  ...
  return (
    <article className="rounded-lg border border-border bg-card p-4">
      ...
      <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-sm">
        ...
        <span className="tabular-nums text-foreground">{formatBRL(a.priceBRL)}</span>
      </div>
    </article>
  );
}
