...
import { getClientById, MOCK_CLIENTS } from "@/data/admin/clients";
import { getAppointmentsByClient } from "@/data/admin/appointments";
...
export default async function ClientDetailPage({
  ...
  const preferredBarber = client.preferredBarberId
    ? getBarberById(client.preferredBarberId)
    : null;
  const preferredService = client.preferredServiceId
    ? getServiceById(client.preferredServiceId)
    : null;
  ...
  const stats = [
    ...
    },
    {
      icon: <TrendingUp className="size-4" />,
      label: "Total gasto",
      value: formatBRL(client.totalSpentBRL),
    },
    {
      icon: <Award className="size-4" />,
      label: "Barbeiro preferido",
      value: preferredBarber?.name ?? "—",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle className="font-display text-lg tracking-[0.04em]">
              Contato
            </CardTitle>
          </CardHeader>
        ...
        <div className="grid grid-cols-2 gap-3 lg:col-span-2">
          {stats.map((s) => (
            ...
            >
              ...
              <div className="mt-2 font-display text-2xl tracking-[0.04em] text-foreground">
                {s.value}
              </div>
            </div>
          ))}
              ...
              {history.map((a) => (
                <li key={a.id}>
                  <article className="rounded-lg border border-border bg-card p-4">
                    ...
                    <div className="mt-2 border-t border-border pt-2 text-sm tabular-nums text-foreground">
                      {formatBRL(a.priceBRL)}
                    </div>
                  </article>
                </li>
